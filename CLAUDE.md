...
**CUSTOM components** carry their own live `circuit` instance (deep-cloned on instantiation). Evaluation is recursive through the hierarchy.
...
**Junctions (`JUNCTION`):** a bus tap — one node (pin 0) that merges everything wired *into* it (resolved by `busValue` in `passCircuit`) and fans its value out to anything wired *from* it. Junctions chain (junction→junction). In the UI, dropping a wire onto a junction always merges (no Shift); `hitPin` returns kind `"j"` for them and tapping one starts a wire *from* it.
